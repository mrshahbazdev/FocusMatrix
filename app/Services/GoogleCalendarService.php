...
class GoogleCalendarService
{
    ...
    public function upcomingEvents(Integration $integration, int $hours = 72): array
    {
        try {
            ...
            Log::warning('Google Calendar fetch failed', ['error' => $e->getMessage()]);
            return [];
        }
    }
    ...
    public function createFocusBlock(Integration $integration, string $title, Carbon $start, int $minutes = 60): ?string
    {
        try {
            ...
            Log::warning('Google Calendar insert failed', ['error' => $e->getMessage()]);
            return null;
        }
    }

    /**
     * Heuristic meeting auditor: returns the reasons a meeting looks weak.
     */
